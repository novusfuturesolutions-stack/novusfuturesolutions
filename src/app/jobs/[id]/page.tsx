...
export default function JobDetailPage() {
  ...
  if (!job) {
    return (
      <div className="min-h-screen bg-slate-50 text-slate-900 py-20 px-4 text-center space-y-4">
        <h2 className="text-2xl font-bold text-slate-900">Job Opening Not Found</h2>
        <p className="text-xs text-slate-500">The vacancy you are looking for may have expired or been removed.</p>
        <Link href="/jobs" className="inline-block px-4 py-2 bg-blue-600 text-white font-bold text-xs rounded-xl">
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 py-10 px-4 sm:px-6 lg:px-8 space-y-8">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        {/* Back Link */}
        <Link href="/jobs" className="inline-flex items-center gap-1.5 text-xs text-slate-500 hover:text-slate-900 transition-colors font-bold">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              <img
                src={job.companyLogo}
                alt={job.companyName}
                className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl object-cover ring-1 ring-slate-200 bg-slate-50"
              />
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">{job.title}</h1>
                  {job.urgentHiring && (
                    <span className="px-2.5 py-0.5 rounded-full bg-orange-100 text-orange-700 border border-orange-200 text-[10px] font-bold uppercase tracking-wider flex items-center gap-1">
                      <Zap className="w-3 h-3 fill-orange-600" /> Urgent
                    </span>
                  )}
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            <div className="bg-white p-6 sm:p-8 rounded-3xl space-y-6 border border-slate-200/80 shadow-sm text-xs sm:text-sm text-slate-700 leading-relaxed">
              ...
              {job.requiredLicenses.length > 0 && (
                <div className="pt-4 border-t border-slate-100 space-y-2.5">
                  <h3 className="font-bold text-slate-900 text-xs uppercase tracking-wider">Mandatory Certificates / CDL</h3>
                  <div className="flex flex-wrap gap-2">
                    {job.requiredLicenses.map((lic, i) => (
                      <span key={i} className="px-3 py-1 rounded-lg bg-blue-50 text-blue-700 border border-blue-200/70 text-xs font-semibold">
                        ✓ {lic}
                      </span>
                    ))}
                  </div>
              ...
              <div className="pt-4 border-t border-slate-100 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {job.benefits.map((b, i) => (
                    <div key={i} className="p-3 rounded-xl bg-slate-50 border border-slate-200/80 flex items-center gap-2 text-xs">
            ...
            <div id="apply-section" className="bg-white p-6 sm:p-8 rounded-3xl border border-blue-200 shadow-md space-y-6">
              ...
              {!user ? (
                <div className="rounded-2xl border border-blue-200 bg-blue-50 p-6 text-center">
                  ...
                  <Link href={`/auth?redirect=${encodeURIComponent(`/jobs/${job.id}#apply-section`)}`} className="mt-4 inline-flex rounded-xl bg-blue-600 px-5 py-3 text-xs font-black text-white">Sign in / Sign up</Link>
                </div>
              ) : applied ? (
                ...
                <form onSubmit={handleApply} className="space-y-4 text-xs">
                  <div className="space-y-1.5">
                    ...
                    <textarea
                      ...
                      className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 placeholder-slate-400 focus:outline-none focus:border-blue-600 focus:bg-white transition-colors"
                    />
                  </div>
                  ...
                  {job.screeningQuestions?.map((q, i) => (
                    <div key={i} className="space-y-1.5">
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 placeholder-slate-400 focus:outline-none focus:border-blue-600 focus:bg-white transition-colors"
                      />
                    </div>
                  ))}
            ...
            <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-sm space-y-4 text-xs">
              <div className="flex items-center gap-3">
                <img
                  src={job.companyLogo}
                  alt={job.companyName}
                  className="w-12 h-12 rounded-xl object-cover ring-1 ring-slate-200 bg-slate-50"
                />
                <div>
              ...
              <Link
                ...
                className="block w-full py-2.5 text-center bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold rounded-xl transition-colors"
              >
